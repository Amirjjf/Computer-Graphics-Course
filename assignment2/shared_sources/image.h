...
using Eigen::Vector, Eigen::Vector2i, Eigen::Vector2f;
...
public:
    ...
    std::shared_ptr<ImageBase<Vector<uint8_t, 4>>> to_uint8() const;

    void exportPNG(const std::filesystem::path& filename) const
    {
        std::shared_ptr<ImageBase<Vector<uint8_t, 4>>> u8img = to_uint8();
    ...
    inline PixelType*           data()                          { return data_.data(); }
...
template<class PixelType>
std::shared_ptr<ImageBase<Vector<uint8_t, 4>>> ImageBase<PixelType>::to_uint8() const
{
    Vector<uint8_t, 4> initializer = Vector<uint8_t, 4>{ 0, 0, 0, 255 };
    auto u8img = std::make_shared<ImageBase<Vector<uint8_t, 4>>>(size_, initializer);
    for (int j = 0; j < size_(1); ++j)
        for (int i = 0; i < size_(0); ++i)
            for (int d = 0; d < std::min(4, (int)PixelType::RowsAtCompileTime); ++d)
                if ( std::is_floating_point_v<decltype(pixel(i, j)[d])>)
                    u8img->pixel(i, j)[d] = uint8_t(std::max(0.0f, std::min(255.0f, pixel(i, j)[d]*255.0f)));
                else
                    u8img->pixel(i, j)[d] = pixel(i, j)[d];
                //u8img->pixel(i, j)[d] = pixel_traits_t<PixelType::Scalar>::to_uint8(pixel(i, j)[d]);
    return u8img;
}
